...
TS_star = 36
pixel_firing = EPLS.get_firing_pixels(TS_star, filter=False)
print(len(pixel_firing))

# Compute the TS_star from a chosen QF and alpha
QF = 0.5
alpha = 0.05
TS_star = EPLS.get_TS_from_QF(QF, alpha=alpha)
pixel_firing = EPLS.get_firing_pixels(TS_star, filter=False)
print(len(pixel_firing))
```
...
```python
TS_star=36
firing_pixels = EPLS.get_firing_pixels(TS_star, filter=False) 
TS_ranking = EPLS.TS_map_Fermi[firing_pixels]
```

And we can compute the QF and QF range of the firing pixels, given alpha:
```python
# obtain the QF using all the simulations
QF = EPLS.get_QF_from_TS(TS_ranking, alpha=alpha)

# compute the mean and std of the QF using batches of simulations
mean_QF, std_QF = EPLS.get_QF_ranges_from_TS(TS_ranking, alpha=alpha, 
...
pixel_firing = EPLS.get_firing_pixels(TS_star, filter=True)
...
pixel_firing = EPLS.get_firing_pixels(TS_star, filter=True, conservative=True)
...
pixel_firing = EPLS.get_firing_pixels(TS_star, filter=True, conservative=True, deg=0.5)
```
